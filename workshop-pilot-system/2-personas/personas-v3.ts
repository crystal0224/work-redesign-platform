...
export const PERSONAS_V3: Persona[] = [
...

  {
    ...
    work: {
      ...
      painPoints: [
        '시니어 5명이 CRM 시스템(Salesforce)에 고객 정보를 제대로 입력하지 않아서, 팀장이 실제 영업 현황을 파악하려면 일일이 전화로 물어봐야 함. 예를 들어 월요일 오전 10시 주간 파이프라인 회의에서 "이번 주 예상 계약 건수가 몇 건이죠?"라고 물으면, 시니어들은 "제가 다 머릿속에 있어요. 아마 3-4건 될 것 같은데요"라는 막연한 답변만. CRM에는 최소한의 고객사 이름과 금액만 입력되어 있고, 언제 제안했는지, 고객 반응이 어땠는지, 다음 미팅 일정이 언제인지는 다 비어 있음. 결국 팀장이 시니어 5명에게 각각 전화해서 "S전자 건은 어떻게 됐어요?" 물어봐야 하는데, 12년차 시니어는 "아, 그거요? 다음 주에 다시 연락하기로 했어요"라고만 답변. 주니어들은 이런 노하우를 전혀 못 배우고, 팀장도 정확한 영업 현황 파악이 안 돼서 본부장 보고 때마다 불안',
        '제안서 템플릿이 없어서 팀원마다 제각각이고, 주니어가 만든 제안서를 팀장이 70% 다시 만드는 경우가 많아서 주당 10시간 소요. 예를 들어 지난주 화요일, 주니어 영업(2년차)이 중견기업 제안서를 3일 동안 30페이지 PPT로 만들어왔는데, 팀장이 검토해보니 "우리 제품 스펙만 나열했지 고객 니즈가 반영 안 됐네"라며 15페이지를 통째로 다시 작성. 금요일 PT인데 목요일 밤까지 수정 반복. 결국 팀장이 목요일 밤 10시까지 남아서 직접 완성. 주니어는 "제가 쓸모없나봐요"라며 의욕 상실, 팀장은 "내가 다 해줘야 하나"라며 지침. 시니어들은 각자 본인만의 제안서 양식을 쓰는데, A는 회사 소개를 앞에 넣고, B는 고객 니즈 분석부터 시작하고, C는 기술 스펙 중심. 통일된 템플릿이 없어서 팀의 브랜드가 일관성 없음',
        '고객사 미팅 후 follow-up을 누가 언제 해야 하는지 애매해서, 시니어는 "제가 알아서 해요"라고 하고 주니어는 눈치만 보다가 결국 팀장이 리마인드하는데 놓치는 고객 발생. 지난달 수요일 오전, 7년차 시니어가 A기업과 미팅 후 "다음 주에 연락하기로 했어요"라고 보고. 팀장이 다음주 월요일에 "A기업 follow-up 하셨어요?"라고 물으니 "아, 깜빡했네요. 오늘 할게요"라는 답변. 그 사이 경쟁사가 먼저 제안서를 제출해서 기회 놓침. 주니어 4명은 시니어 눈치 보면서 "제가 follow-up 전화 드릴까요?"라고 물어보는데, 시니어는 "괜찮아, 내가 할게"라고만 하고 실제로는 안 함. 팀장이 매일 "누가 누구한테 follow-up 해야 하는지" 체크리스트를 만들어 관리하지만, 시니어들은 "그런 거 안 해도 제가 알아서 해요"라며 무시. CRM에도 follow-up 일정이 입력 안 돼서 팀장이 수시로 확인해야 함',
        '월말 실적 집계할 때 각자 엑셀로 보고해서 팀장이 수작업으로 합산하는 데 3-4시간 소요. 시니어 5명은 각자 본인만의 엑셀 양식을 쓰는데, A는 "고객사명 | 계약금액 | 확률" 형식이고, B는 "업종 | 금액 | 단계" 형식, C는 아예 이메일로 "이번 달 제 실적은 3건, 총 15억입니다"라고 텍스트만 보냄. 주니어 4명은 CRM에서 데이터를 뽑아 Excel로 보내는데, 열 이름이 제각각. 팀장이 금요일 오후 2시부터 이 데이터들을 하나하나 복사해서 마스터 Excel에 붙여넣고, 피벗 테이블 만들어서 팀 전체 실적 정리. 중간에 12년차 시니어 데이터에 "확률 70%"라고 적혀 있길래 "이거 확실한 거죠?"라고 확인 전화했더니 "아, 그거 고객이 아직 고민 중이에요. 50%로 보는 게 맞을 것 같네요"라며 숫자 수정. 결국 금요일 오후 6시까지 걸려서 완성. 본부장은 "금요일 오전까지 주간 리포트 달라"고 하는데 불가능',
        '시니어들끼리는 경쟁 의식 때문에 고객 정보를 공유 안 하고, 같은 고객사에 중복 접촉하는 경우도 있음. 지난달 분기 마감 주 목요일, S전자 구매팀에서 본부장한테 직접 전화가 옴. "SK텔레콤에서 같은 제안을 두 팀원이 따로 보냈는데, 내부 소통이 안 되나요?"라는 불만. 알고 보니 7년차 시니어와 10년차 시니어가 각각 S전자 다른 부서를 담당하는데, 서로 "내 고객"이라고 생각해서 동시에 제안서 제출. 고객 입장에서는 혼란스럽고 "SK텔레콤이 정리가 안 된 회사"라는 이미지. 팀장이 두 시니어를 불러서 "왜 사전에 공유 안 했어요?"라고 물으니, 7년차는 "제가 먼저 관계 맺었어요", 10년차는 "제 부서가 더 큰 계약이에요"라며 양보 안 함. 결국 본부장이 개입해서 10년차 담당으로 정리했지만, 7년차는 "내 실적 뺏겼다"며 불만. 팀장 입장에서는 "CRM에 고객 정보 제대로 입력하고 공유했으면 이런 일 안 생겼을 텐데"라고 생각하지만, 시니어들은 "내 고객 정보가 공개되면 다른 사람이 가로챌까봐"라는 경계심. 팀 내 협업 문화가 무너짐'
      ],
      automationNeeds: [
...

  {
    ...
    work: {
      ...
      painPoints: [
        '3교대 조장 8명 중 2명이 본인보다 연차가 높아서 (15년차, 20년차) 생산 방식 바꾸자고 하면 "우리는 원래 이렇게 했다"며 저항하고, 신임 팀장이라 강하게 못 나감. 예를 들어 지난달 화요일 오전 11시, 공장장이 "생산성 향상 방안 내주세요"라고 지시해서 팀장이 "MES 시스템에 실시간 데이터 입력해서 병목 구간 파악하자"고 제안했는데, 20년차 조장이 "팀장님, 그거 입력하느라 생산 늦어지면 어떡하려고요? 우리는 20년 넘게 이 방식으로 했는데 문제없었어요"라고 공개적으로 반박. 다른 조장 7명도 20년차 눈치 보면서 침묵. 팀장은 "그래도 데이터 기반으로..."라고 말하려다 "팀장님은 현장을 잘 모르시는 것 같네요"라는 말에 할 말이 없어짐. 결국 "조장님들과 더 논의해보겠습니다"라고 물러섰는데, 공장장한테는 "개선 방안이 왜 안 나오냐"고 질책받고, 조장들한테는 "신임이라 현장을 몰라"는 평가. 리더십이 서지 않음',
        '야간조에서 문제 생기면 새벽 2-3시에 전화 오는데, 팀장 혼자 판단해야 하는데 현장 상황을 정확히 모를 때가 많고 조장한테 물어보면 "팀장님이 결정하세요"라고만 함. 지난주 목요일 새벽 2시 30분, 야간조장(15년차)에게서 전화. "팀장님, 3번 라인 진동 센서에 이상 신호 떴는데 라인 멈출까요? 아니면 계속 돌릴까요?" 팀장은 침대에서 일어나 "진동 수치가 얼마예요?"라고 물었더니 "저도 정확히는 모르겠는데 경고등이 깜빡거려요"라는 답변. "조장님 판단에는 어떤 게 나을 것 같아요?"라고 물으니 "팀장님이 결정하셔야죠. 제가 판단했다가 잘못되면 책임 못 져요"라고 떠넘김. 팀장은 현장을 못 가보니 상황 파악이 안 돼서 "일단 멈추고 설비팀 불러주세요"라고 지시. 오전 7시 출근해서 보니 센서 오작동이었고 실제로는 문제없었음. 공장장이 "왜 생산 5시간 차질 났냐? 야간에 판단 제대로 못하냐"고 질책. 팀장은 현장도 못 보면서 새벽에 중대 결정을 내려야 하는 공포가 반복됨',
        '생산 데이터가 MES(제조실행시스템), 엑셀, 수기 장부에 다 흩어져 있어서 주간 보고서 만들 때 데이터 취합에 금요일 오후 5시간 걸림. 주간조는 MES 시스템에 생산량을 입력하는데, 저녁조는 "MES가 느리다"며 엑셀에 따로 기록하고, 야간조는 "컴퓨터 켜기 귀찮다"며 수기 장부에 볼펜으로 작성. 금요일 오후 1시, 팀장이 이번 주 생산 실적을 정리하려고 MES 데이터를 다운받았더니 저녁조 데이터가 2일치 빠져 있음. 저녁조장한테 전화했더니 "아, 제가 엑셀에 적어뒀어요. 이따 보내드릴게요"라는 답변. 2시간 후 엑셀 파일이 왔는데 열 이름이 MES와 달라서 수동으로 복사 붙여넣기. 야간조는 수기 장부를 사진 찍어 카톡으로 보냈는데, 글씨가 흘려 써서 "7,850"인지 "7,350"인지 판독 불가. 다시 전화해서 확인. 이렇게 3교대 데이터를 취합하고 피벗 테이블 만들어 정리하는 데 금요일 오후 6시까지 소요. 공장장은 "금요일 오전까지 주간 보고서 달라"고 하는데, 불가능. 결국 목요일 밤 야근해서 미리 준비',
        '설비 고장 예측이 안 돼서 갑자기 라인 멈추면 그제서야 설비팀 부르는 사후 대응만 반복. 지난주 수요일 오후 2시, 2번 라인이 갑자기 멈춤. 조장이 달려가서 확인했더니 "모터 과열"이라는 에러 메시지. 급하게 설비팀에 전화했더니 "지금 다른 라인 고치는 중이라 30분 후 갈게요"라는 답변. 결국 2번 라인이 1시간 30분 정지. 하루 생산 목표 1만 장에서 800장 부족. 공장장이 "왜 사전에 점검 안 했어?"라고 질책했는데, 팀장도 "모터 과열을 어떻게 미리 알아요?"라고 되묻고 싶지만 참음. 20년차 조장은 "모터 소리가 좀 이상하긴 했는데, 그래도 돌아가길래 그냥 뒀어요"라고 사후 보고. 팀장이 "그럼 그때 말씀해주셨어야죠"라고 하니 "원래 좀 그러다가 괜찮아지거든요. 제 경험상 큰 문제 아니었어요"라는 답변. 조장들의 "경험"과 "감"으로만 설비를 관리하다 보니, 예측 불가능한 고장이 반복되고 팀장은 항상 사후약방문',
        '주니어 14명 교육을 시니어 조장들한테 맡기는데, 조장마다 가르치는 방식이 달라서 주니어들 혼란스럽고 표준 교육 자료가 없음. 지난달 신입 생산기사 3명이 입사했을 때, 팀장이 주간조장(12년차), 저녁조장(10년차), 야간조장(15년차)에게 각각 1명씩 교육 맡김. 2주 후 팀장이 신입 3명을 모아서 "웨이퍼 검사 방법 배웠죠?"라고 물었더니, A는 "육안으로 먼저 보고 이상 있으면 현미경 검사한다고 배웠어요", B는 "무조건 현미경으로 전수 검사한다고 배웠어요", C는 "샘플링해서 10개 중 1개만 검사한다고 배웠어요"라는 서로 다른 답변. 조장마다 본인 스타일대로 가르친 거였음. 팀장이 조장 3명을 불러서 "표준 작업 지침서대로 가르쳐주세요"라고 했더니, 15년차 조장이 "지침서는 이론이고 현장은 달라요. 제 방식이 더 효율적이에요"라고 답변. 결국 신입 3명은 누구 말을 따라야 할지 몰라 혼란스럽고, 품질 편차 발생. 표준 교육 자료를 만들려고 해도 조장들이 "제 노하우를 왜 문서화해야 하냐"며 협조 안 함'
      ],
      automationNeeds: [
  ...
  // ==================== SALES (2명) ====================
  {
    ...
    work: {
      ...
      painPoints: [
        '신규 리드 발굴을 팀원 10명이 LinkedIn, 업계 뉴스, 전시회 등에서 수작업으로 하느라 영업 시간 부족. 예를 들어 주니어 영업 9명은 매일 오전 1-2시간을 LinkedIn에서 잠재 고객 검색하고 수기로 Excel 명단 작성하는 데 쏟는데, 이렇게 찾은 리드의 70%는 이미 다른 팀원이 접촉한 중복 고객이거나 구매 의사 없는 콜드 리드. 1등 영업 2명은 "그럴 시간에 기존 고객 관리하는 게 낫다"며 신규 발굴 안 함. 결국 주니어들은 시간만 쓰고 성과 없고, 팀 전체 신규 고객 비율은 분기당 5%에 불과. 본부장은 "왜 신규 고객 안 늘어나냐"고 압박하지만, 비효율적인 리드 발굴 방식 때문에 실제 영업 활동 시간은 부족',
        '제안서 작성에 팀원들 시간의 30%가 소모되어 실제 영업 활동 시간이 크게 부족. 월 평균 15-20개 제안서를 만드는데, 템플릿이 없어서 매번 처음부터 작성. 주니어가 만든 제안서를 시니어가 검토하면 "이건 아니지"라며 70% 재작성. 팀장도 최종 검토 시 "고객사 니즈가 반영 안 됐네"라며 다시 수정 지시. 한 건당 평균 3-4회 수정 반복해서 완성까지 1주일 소요. 지난주 화요일, 대형 고객사 제안 요청 들어왔는데, 주니어가 3일 동안 40페이지 PPT 만들었더니 팀장이 "고객이 원하는 건 이게 아니야. 다시 해"라고 지시. 결국 팀장이 주말에 직접 만들어서 월요일 제출. 주니어는 "제가 쓸모없나봐요"라며 의욕 상실, 팀장은 "내가 다 해줘야 하나"라며 지침',
        '영업 1등 2명(8년차, 12년차)의 노하우가 개인에게만 축적되고 팀 전체에 공유 안 됨. 이들은 개인 네트워크로 연 매출 40억씩 올리는데, "어떻게 계약 따냈어요?"라고 물으면 "그냥 관계 좋아서"라는 막연한 답변만. CRM에 고객 정보 입력도 최소한만 해서, 주니어들은 1등 영업이 어떤 고객과 어떤 대화를 나눴는지 전혀 모름. 팀장이 "노하우 공유 좀 해주세요"라고 부탁하면 "저도 바쁜데요. 그런 거 문서화할 시간 없어요"라며 거부. 주니어들은 "선배들처럼 하고 싶은데 방법을 모르겠어요"라고 하소연하지만, 팀장도 1등 영업한테 강제할 방법이 없음. 결국 주니어는 실적 압박에 시달리고, 팀 전체 역량은 1등 2명에게만 의존하는 구조',
        '월말 실적 압박 시즌에 1등 영업 2명한테 사정하듯 부탁해야 하는 상황이 반복되어 팀장 권위 떨어짐. 지난달 말 금요일 오후 4시, 본부장이 "이번 달 목표 아직 10억 부족한데 어떻게 할 거야?"라고 압박. 급하게 1등 영업(12년차)한테 전화했더니 "저 내일 휴가인데요. 다음 주 월요일에 고객 만나요"라는 대답. "금요일 안에 계약 하나만 더 따줄 수 있을까요?" 사정조로 부탁했더니 "팀장님, 영업이 그렇게 되는 게 아니잖아요. 전 제 일정대로 할게요"라며 거절. 팀장은 할 말이 없고, 결국 본부장한테 "최선을 다했지만 이번 달은 어렵습니다"라고 보고하고 "팀 관리 제대로 해"라는 질책. 주니어들도 이런 상황을 보면서 "1등 선배님들이 팀장보다 파워가 세다"는 인식. 팀장 리더십 손상',
        '영업 파이프라인 예측이 부정확해서 월말에 항상 긴급 상황 발생. CRM에 파이프라인 단계(리드-제안-협상-계약)를 입력하게 하지만, 시니어들은 "70% 확률"이라고 낙관적으로 기록해놔서 실제로는 30%도 안 되는 경우 많음. 팀장은 CRM 숫자만 보고 "이번 달 목표 달성 가능"이라고 판단했다가, 막판에 계약 3건이 동시에 유찰되면서 목표 미달. 본부장한테 "왜 예측을 못해?"라는 질문에 "팀원들이 CRM을 정확히 안 써서..."라고 변명하면 "그럼 팀장이 관리해야지"라는 답변. 정확한 파이프라인 예측 방법이 없어서 매달 불안'
      ],
      automationNeeds: [
...

  {
    ...
    work: {
      ...
      painPoints: [
        '전국 흩어진 15명 담당자와 실시간 소통이 안 돼서 문제 대응이 항상 늦음. 예를 들어 지난주 수요일 오전 11시, 본부장이 "경기 북부 지역 이번 주 매출이 왜 20% 떨어졌어?"라고 긴급 질문했는데, 담당자한테 전화했더니 "잘 모르겠는데요, 매장 돌아보고 오후에 연락드릴게요"라는 답변. 오후 3시에 다시 전화했더니 "매장 사장님들이 경쟁사 프로모션 때문에 우리 제품 재고 안 쌓는대요"라는 보고. 그 사이 4시간 동안 본부장한테 "확인 중입니다"라고만 보고해서 "팀장은 현장 파악도 못하냐"는 질책. 실시간 현황을 못 보니 대응이 항상 사후약방문',
        '매장별 매출 데이터를 15명 담당자가 전날 밤 문자나 Excel 파일로 보내는데, 양식이 제각각이어서 통합 집계하는 데 금요일 오후 3-4시간 소요. A 담당자는 "A매장 100만원, B매장 80만원" 식으로 문자 보내고, B 담당자는 Excel 파일로 보내는데 열 이름이 "매출", "판매액", "합계" 등으로 다 다름. 이걸 하나하나 복사해서 팀장이 마스터 Excel에 붙여넣고 피벗 테이블 만들어 정리. 중간에 숫자 오타 발견하면 담당자한테 다시 전화해서 확인. 본부장은 "금요일 오전까지 주간 리포트 달라"고 하는데 목요일 밤까지 데이터가 안 와서 금요일 새벽 야근 불가피',
        '50대 베테랑 담당자 3명이 디지털 도구에 익숙하지 않아서 새로운 시스템 도입 시 저항이 엄청남. 작년에 간단한 재고 현황 앱을 도입하려고 했는데, 베테랑 담당자가 "난 30년 종이 장부로 했어. 앱은 안 쓸 거야"라고 거부. 젊은 담당자 5명은 잘 쓰는데, 베테랑들이 안 쓰니까 결국 시스템이 이원화돼서 팀장이 두 가지 방식 다 관리해야 함. 베테랑한테 "앱 설치 좀 해주세요"라고 부탁하면 "팀장님, 저 스마트폰도 잘 몰라요. 전화로 보고하면 안 돼요?"라는 답변. 전화로 30분씩 설명해줘도 다음에 또 물어봄. 결국 포기하고 예전 방식 유지',
        '매장 재고 부족 전화가 월말이면 폭탄처럼 쏟아지는데, 팀장이 현장을 직접 못 가보니 담당자 말만 믿고 긴급 발주해야 하는 불안감. 지난달 말 금요일 오후 4시, 담당자가 "C매장 재고 급해요, 월요일까지 100개 발주해주세요"라고 전화. 팀장이 재고팀에 긴급 요청했는데, 월요일에 보니 실제로는 50개면 충분한 상황. 담당자가 과대 요청한 거였는데 팀장은 현장 상황을 모르니 검증 못하고 그대로 처리. 이런 일이 반복되면 재고팀에서 "리테일영업팀은 재고 관리를 못한다"는 평가받고, 팀장도 본부장한테 "팀원 관리 제대로 해"라는 지적받음',
        '50대 베테랑 담당자 3명은 "내가 제일 잘 안다"는 자부심으로 팀장 지시를 형식적으로만 따르고 실제로는 본인 방식대로 일함. 팀장이 "이번 주는 신제품 프로모션 집중하세요"라고 카톡으로 공지했는데, 베테랑은 "제가 30년 현장 다녔는데 제 판단으로 하겠습니다"라고 답하고 기존 제품 프로모션 계속 진행. 나중에 본부장이 "신제품 실적 왜 안 나와?"라고 물으면, 베테랑은 "매장 사장님들이 기존 제품 더 좋아해요"라고 변명. 팀장 입장에서는 본부 방침을 따르라고 강하게 못 나가면 (베테랑이 퇴사하면 200개 매장 중 60개를 관리할 사람이 없음) 본부장한테는 "팀장 관리 능력 부족"이라는 평가. 샌드위치 신세'
      ],
      automationNeeds: [
        '매장별 실시간 재고/매출 대시보드',
        '자동 발주 알림 시스템',
  ...
  // ==================== OPERATIONS (5명) ====================
  {
    ...
    work: {
      ...
      painPoints: [
        '공정별 데이터가 MES, SAP, Excel, 공정모니터링 시스템 4개에 분산되어 통합 현황 파악에 매일 2시간 소요. 예를 들어 지난주 화요일 오전 10시, 본부장이 "어제 야간 불량률이 갑자기 3% 늘었는데 왜?"라고 긴급 질문했는데, 야간조 데이터를 확인하려 했더니 MES에는 생산량만 있고, SAP에는 자재 투입량만 있고, 공정모니터링 시스템에는 온도/압력 센서 데이터만 있고, 불량 상세는 Excel에 있음. 데이터 분석가한테 급하게 "4개 시스템 데이터 통합해서 분석해줘"라고 요청했더니 "통합하려면 2시간 걸려요. 각 시스템에서 데이터 다운받고 Excel로 조인해야 해요"라는 답변. 결국 점심시간이 되어서야 "3번 공정 온도 설정 오류"라는 원인 파악. 본부장이 "왜 실시간 파악이 안 되냐? 다른 회사는 대시보드로 즉시 본다던데"라고 질책. 팀장은 매일 본부 보고 전에 2시간씩 데이터 취합하느라 정작 개선 활동은 못함',
        '불량 원인 분석을 엔지니어 5명에게 맡기는데 수작업으로 Minitab 돌려서 시간 오래 걸림. 월요일 오후 3시, 2번 라인 불량률이 갑자기 5%로 급증. 공정 엔지니어한테 "원인 분석 급해요"라고 요청했더니 "데이터 정리부터 해야 해서 내일 아침까지 걸릴 것 같아요"라는 답변. 엔지니어가 MES에서 24시간치 생산 데이터 5,000건을 Excel로 다운받고, Minitab에 로드해서 공정 파라미터(온도, 압력, 속도) 10개와 불량률 상관관계 분석하는 데 4시간 소요. 화요일 오전 11시에야 "압력 설정 범위가 표준에서 벗어났습니다"라는 분석 결과. 그사이 불량품 500개 더 생산. 본부장이 "왜 실시간으로 못 잡아? AI 같은 거 안 쓰냐?"고 질책했는데, 팀장도 "수작업 분석이라 한계가 있습니다"라고 하소연하고 싶지만 참음. 엔지니어들도 매일 반복적인 데이터 정리에 지쳐서 "이거 자동화 안 되나요?"라고 물어보지만 팀장도 방법을 모름',
        '현장 생산 관리자 10명이 디지털 도구에 익숙하지 않아서 신규 시스템 교육이 어려움. 작년에 MES 모바일 앱을 도입해서 현장에서 태블릿으로 생산 현황 입력하도록 했는데, 50대 관리자 3명이 "태블릿 어떻게 써요? 화면이 자꾸 꺼져요"라며 사용 못함. 팀장이 일주일 동안 매일 30분씩 1:1 교육했지만, 다음 주에 보니 다시 종이에 기록하고 있음. "왜 앱 안 쓰세요?"라고 물으니 "태블릿이 느려서 답답해요. 종이가 더 빨라요"라는 답변. 30대 관리자 7명은 앱을 잘 쓰는데, 50대 3명이 안 쓰니까 결국 종이 기록과 앱 입력을 둘 다 관리해야 하는 이중 부담. 본부장은 "디지털 전환률 100% 달성"을 요구하는데, 팀장은 50대 관리자들한테 강제할 수도 없고 (이분들 경험이 많아서 현장에서 없으면 안 됨), 그렇다고 교육을 더 할 시간도 없음. 24시간 3교대라 10명 전체를 한 자리에 모아서 교육하는 것도 불가능'
      ],
      automationNeeds: [
        'AI 기반 설비 이상 징후 예측',
...

  {
    ...
    work: {
      ...
      painPoints: [
        '15개 물류센터 재고 데이터가 하루 1회만 동기화되어 긴급 상황 대응이 늦음. 예를 들어 지난주 수요일 오후 2시, 대형 고객사에서 "지금 당장 500개 재고 확인해줘"라는 긴급 요청이 왔는데, WMS 시스템은 오전 8시 데이터만 있어서 15개 센터에 일일이 전화해서 확인. 부산센터는 담당자가 외근 중이라 1시간 후에야 답변. 총 3시간 걸려서 고객사한테 "지금은 380개 가능하고 내일 오전에 120개 추가 가능"이라고 답변했는데, 고객사는 "경쟁사는 30분 만에 답변 주던데요"라며 불만',
        '운송 경로 최적화를 운송 관리자 7명이 엑셀로 수동 계산하느라 매일 야근. 지난주 목요일, 고속도로 사고로 서울-부산 경로가 막혀서 대체 경로를 찾아야 했는데, 7명이 각자 담당 노선별로 엑셀에서 거리/시간/유류비를 계산. 김 대리는 "경부고속도로 우회"를 추천하고, 박 과장은 "중부고속도로 경유"를 추천. 결국 밤 10시까지 토론 후 경로 확정했는데, 다음날 "중부고속도로가 유류비는 더 들지만 시간은 30분 빠르다"는 걸 뒤늦게 발견. 팀원들이 "AI가 계산해주면 좋을 텐데"라며 불만',
        'WMS, TMS, SAP 세 시스템을 번갈아 보면서 일하느라 팀원들이 혼란스러워함. 재고 담당자 5명은 "WMS에서는 재고 500개인데 SAP에서는 480개라고 나와요. 어느 게 맞아요?"라고 매일 물어봄. 지난주 금요일, 본부장한테 "이번 달 물류비가 작년 대비 12% 증가했습니다"라고 보고했더니 "왜 증가했는지 원인 분석해"라는 지시. TMS에서 운송비 데이터, SAP에서 인건비 데이터, WMS에서 보관비 데이터를 각각 다운받아서 엑셀로 수동 취합. 데이터 형식이 달라서 정리하는 데만 3일 걸림. 월요일에 "유류비 8% 증가, 긴급 배송 횟수 15% 증가가 원인"이라고 보고했지만 확신이 없어서 떨림'
      ],
      automationNeeds: [
        '실시간 재고 통합 모니터링',
        'AI 기반 운송 경로 자동 최적화',
...

  {
    ...
    work: {
      ...
      painPoints: [
        '일 5,000개 웨이퍼 검사 데이터 분석을 엔지니어 4명이 수작업으로 Minitab 돌려서 하느라 일 2-3시간 소요. 검사원 6명이 품질검사 장비로 웨이퍼를 검사하면 데이터가 장비 SW에 저장되는데, 이걸 엔지니어들이 Excel로 다운받아서 Minitab에 로드하고, 통계 분석(공정 능력 지수 Cpk, 불량률 추이, 파레토 차트) 돌리는 데 1명당 평균 2-3시간 소요. 지난주 월요일, 본부장이 "금요일 불량률 추이 분석해서 보고해"라고 지시했는데, 엔지니어 A가 "금요일 데이터 5,000건을 정리하고 분석하는 데 오늘 오후까지 걸릴 것 같아요"라는 답변. 결국 오후 5시에야 "금요일 불량률이 평소 2.1%에서 2.8%로 증가했습니다"라고 보고. 본부장이 "왜 그렇게 오래 걸려? 다른 팀은 실시간 대시보드로 즉시 본다던데"라고 질책. 엔지니어들도 매일 반복적인 데이터 정리 작업에 지쳐서 "이거 자동화 안 되나요?"라고 하소연하지만 팀장도 방법을 모름',
        '불량 패턴 발견이 사후적이라 예방 못하고 이미 불량품이 수백 개 나온 후에야 원인 파악. 지난달 수요일 오후 3시, 3번 공정에서 불량률이 갑자기 5%로 급증. 엔지니어한테 "원인 분석 급해요"라고 요청했더니 "과거 3일치 데이터를 분석해봐야 패턴이 보일 것 같아요"라는 답변. 목요일 오전까지 분석한 결과 "화요일부터 온도 설정이 조금씩 벗어나기 시작했네요. 패턴상 목요일에는 8%까지 올라갈 것 같아요"라는 분석. 이미 수요일에 불량품 250개, 목요일 오전에 150개 더 생산. 팀장이 급하게 공정 중단하고 온도 재설정. 본부장이 "왜 화요일에 미리 못 잡았어? 불량품 400개 손실이 얼마인지 알아?"라고 질책. 팀장은 "화요일엔 불량률이 2.3%로 정상 범위(2.5% 이하)여서 이상 징후를 몰랐습니다"라고 변명하지만, 본부장은 "AI로 패턴 감지하면 미리 잡을 수 있다던데"라고 압박',
        '고객 클레임 시 과거 품질 데이터를 SAP QM, 검사 장비 SW, Excel 3개 시스템에서 찾느라 팀원 3명이 오전 내내 소요하고 스트레스 받음. 지난주 월요일 오전 11시, 고객사(삼성전자)에서 "지난달 15일 납품분 LOT#A12345에서 불량 발견했어요. 같은 패턴이 다른 LOT에도 있는지 즉시 확인해주세요"라는 긴급 요청. 팀장이 엔지니어 3명한테 "LOT#A12345 전후 10일치 데이터 찾아서 패턴 비교해"라고 지시. 엔지니어들이 SAP QM에서 LOT 이력 조회하고, 검사 장비 SW에서 해당 날짜 검사 데이터 다운로드하고, Excel에 정리된 불량 상세 기록 찾는 데 오전 내내 소요. 데이터가 3개 시스템에 흩어져 있어서 "이 LOT는 SAP에 없는데 Excel에는 있네요. 날짜가 틀린 건가?"라며 혼란. 오후 3시에야 "같은 패턴이 LOT#A12340, A12348에도 있습니다"라고 고객사에 답변. 고객사가 "왜 4시간이나 걸려요? 즉시 조회 안 돼요?"라며 불만. 팀원들은 "과거 데이터 찾을 때마다 스트레스받아요. 통합 DB 있으면 5분이면 될 텐데"라고 하소연'
      ],
      automationNeeds: [
        'AI 기반 불량 패턴 자동 감지',
        '검사 데이터 자동 분석 및 리포팅',
...

  {
    ...
    work: {
      ...
      painPoints: [
        '직원 고충 접수가 이메일/전화/방문으로 분산되어 체계적 관리 안 됨. 어떤 직원은 이메일로 신청, 어떤 직원은 팀장 사무실 직접 방문, 어떤 직원은 전화로 고충 토로. 이걸 노무 담당 2명이 각자 종이 파일에 기록하는데, 파일이 따로따로 보관돼서 "누가 무슨 고충 신청했더라?" 찾기 어려움. 지난주 화요일 오전 11시, 현장 직원이 팀장 사무실에 직접 찾아와서 "지난달 신청한 고충처리 어떻게 됐나요?"라고 질문. 팀장이 담당자에게 전화했더니 "종이로 받았는데 어디 갔더라..."라는 답변. 30분 동안 서류철 뒤져서 찾았는데, 처리 진행 상황이 기록되지 않아서 "확인해보고 연락드리겠습니다"라고 답변. 직원은 불만족스러운 표정으로 돌아가고, 팀장은 "이래서 안 되는데..."라는 무력감',
        '안전보건 점검 기록이 종이 문서라 분실 위험 있고 통계 내기 어려움. 매주 화요일 현장 안전 라운딩 시 안전보건 담당 1명이 점검표를 종이로 작성 (20-30장). "소화기 위치 확인", "비상구 정상 작동", "안전모 착용률" 등 체크. 이걸 사무실로 가져와서 서류철에 보관하는데, 분기말에 노동청 감사 준비할 때 "3개월 전 점검표 어디 갔지?" 찾느라 비상. 지난 분기, 안전보건 점검표 일부가 분실돼서 노동청 감사에서 지적받음. "점검 안 한 거 아니냐"는 질문에 "했는데 서류를 못 찾겠습니다"라고 답변할 수밖에 없어서 회사 평판 손상. 또한 통계 내려고 하면 종이 문서 20-30장을 일일이 Excel에 수기 입력해야 해서 하루 종일 걸림. 노조가 "안전사고 왜 또 발생했어?"라고 물으면 "과거 점검 기록 찾아보고 답변드리겠습니다"라고 해야 하는데 즉답 불가',
        '복리후생 신청 현황을 Excel로 관리해서 실시간 파악 불가. 직원들이 "경조사 지원금 신청", "건강검진 신청", "사내 동호회 지원" 등 복리후생을 이메일로 신청하면, 복리후생 담당 1명이 Excel에 수기 입력. "신청자", "신청 항목", "승인 여부", "지급 날짜" 등 열을 만들어서 관리하는데, Excel 파일이 개인 PC에만 저장돼 있어서 담당자가 휴가 가면 다른 사람이 현황 파악 못함. 경영진이 "이번 달 복리후생 예산 얼마나 썼어?"라고 물으면 담당자한테 전화해서 "Excel 열어서 합계 좀 알려주세요"라고 부탁. 담당자가 외근 중이면 "돌아와서 확인해드리겠습니다"라고 답변. 실시간 현황 파악 안 돼서 예산 초과 위험 있는데도 사후에야 알게 됨',
        '노사 이슈가 터지면 과거 협의 기록 찾느라 비상. 노조와 근로조건 협상할 때 "3년 전 합의 사항이 뭐였죠?"라고 물으면, 노무 담당 2명이 Word 문서 파일 뒤져서 찾아야 함. 파일명이 "노사협의_20XX_XX.docx" 같은 식으로 날짜만 있어서 내용 검색 어려움. "OOO 건에 대해 어떻게 합의했더라?" 찾으려면 파일 10-20개 일일이 열어봐야 함. 지난달, 노조가 "야근수당 계산 방식 문제 있다"고 제기했는데, 팀장이 "예전에 이 건으로 합의했던 게 있는데..."라고 기억은 나는데 문서를 못 찾음. 결국 "확인 후 답변드리겠습니다"라고 하고 이틀 동안 과거 문서 뒤짐. 노조는 "HR팀이 기록 관리도 제대로 안 하냐"며 불신. 8년차 팀장인데 종이 문서와 Word 파일로만 관리하는 게 뒤처진 느낌'
      ],
      automationNeeds: [
        '고충 처리 시스템(접수/진행/완료)',
        '안전보건 디지털 점검 시스템',
...

  {
    ...
    work: {
      ...
      painPoints: [
        '직원 만족도 조사 결과를 SurveyMonkey에서 Excel로 다운받아 수작업으로 분석하느라 인사이트 도출이 이틀씩 걸림. 분기별로 전 직원 대상 만족도 조사 실시하는데, 응답률 70% (350명). SurveyMonkey에서 CSV 파일 다운받아서 Excel로 열면 "부서", "직급", "만족도 점수" 등 열이 있지만, 부서별/직급별 평균을 구하려면 피벗 테이블 만들고 그래프 그려야 함. 지난주 금요일 오후 2시, 경영진이 "이번 분기 조직문화 프로그램 효과 보고해줘"라고 긴급 요청. 팀장이 SurveyMonkey 결과 다운받아서 Excel로 피벗 테이블 만들고, Slack/이메일/게시판에 올라온 직원 의견 일일이 복사해서 정리하고, 부서별 참여율을 전화로 물어봐서 취합. 이틀 동안 작업해서 PPT 만들어 보고했는데, 경영진이 "그래서 ROI가 얼마야?"라는 질문에 답 못함. "만족도 4.2점"은 좋은데 "실제 이직률 감소", "생산성 향상" 같은 정량적 지표와 연결 못함',
        '사내 소통 채널이 Slack, 이메일, 게시판, Instagram 등 여러 곳에 분산되어 직원 의견을 통합 관리하기 어려움. 직원들이 Slack DM으로 "회사 문화 개선 의견 있어요", 이메일로 "워라밸 제도 건의합니다", 게시판에 "동호회 활동 지원 부탁드려요"라고 올리면, 커뮤니케이션 담당 1명이 일일이 복사해서 Notion에 정리. 그런데 Notion 페이지가 "Slack 의견", "이메일 의견", "게시판 의견"으로 따로따로 있어서, "전체 직원 의견 트렌드"를 한눈에 보기 어려움. 경영진이 "요즘 직원들이 뭘 원해?"라고 물으면, 팀장이 Notion 페이지 3개 다 열어서 스크롤하면서 "음... Slack엔 워라밸 얘기 많고, 이메일엔 복리후생 건의 많고, 게시판엔 사내 카페 개선 요청 많네요"라고 답변. 통합 대시보드 없어서 실시간 트렌드 파악 어려움',
        '문화 프로그램 참여율 데이터가 부서별로 흩어져 있어서 집계 어렵고, 참여 독려도 수작업. 월간 이벤트 (예: 워라밸 데이, 사내 동호회 활동) 진행 후 참여자 명단을 Excel로 정리하는데, "부서", "이름", "참여 여부"만 기록. 부서별 참여율을 알려면 부서장들한테 전화해서 "귀 부서 직원 몇 명 참여했나요?"라고 물어봐야 함. 10개 부서장에게 일일이 전화하는 데 금요일 오후 2시간 소요. 어떤 부서는 "잘 모르겠는데요, 직원들한테 물어보고 연락드릴게요"라고 해서 답변이 늦음. 경영진이 "어느 부서 참여율 왜 낮아?"라고 물으면 즉답 못하고 "확인해보겠습니다"라고 답변. 또한 참여율 낮은 부서에 독려 메일 보내는 것도 수작업이라 시간 소요',
        '조직문화 프로그램 효과를 정량화하지 못해서 경영진에게 ROI 증명 어려움. 예산 3억 쓰면서 "이벤트 만족도 4.2점", "참여율 70%" 같은 정성적 지표만 보고하면, 경영진이 "그래서 회사에 실제로 얼마나 도움 됐어? 이직률은 줄었어? 생산성은 올랐어?"라고 질문. 조직문화와 이직률/생산성의 인과관계를 데이터로 보여주고 싶은데, 조직문화팀은 만족도 조사만 하고, 이직률은 HR 기획팀이 관리하고, 생산성은 각 부서가 관리해서 데이터 연계 안 됨. HR 기획팀한테 "이직률 데이터 좀 주세요"라고 부탁하면 "왜 필요하세요?"라고 물어보고, "조직문화 효과 측정하려고요"라고 답하면 "그건 저희 KPI가 아니라 협조 어려워요"라는 답변. 결국 조직문화 프로그램 예산 삭감 위험'
      ],
      automationNeeds: [
        '만족도 조사 자동 분석 및 시각화',
        '통합 커뮤니케이션 대시보드',
...

  {
    ...
    work: {
      ...
      painPoints: [
        '시장 조사 데이터를 3개 컨설팅사 리포트에서 수작업으로 비교 분석하는 데 분기당 20시간 소요. 보상 설계 담당 2명이 머서, 타워스왓슨, 윌리스타워스왓슨에서 PDF 리포트를 받아서, "직급별 시장 급여 수준", "산업별 연봉 인상률", "복리후생 트렌드" 등 데이터를 Excel에 수작업으로 입력. 지난주 목요일 오전 10시, 경영진이 "우리 회사 급여 수준이 시장 대비 어떤가요?"라고 질문. 팀장이 보상 설계 담당 2명에게 "3개 컨설팅사 시장 조사 리포트 비교 분석해주세요"라고 지시. 2명이 각각 PDF 리포트를 다운받아서 Excel에 수작업으로 입력하고 비교하는 데 이틀 소요. "영업 직군 과장급 시장 평균은 6,500만원인데 우리는 6,200만원이네요" 같은 식으로 일일이 비교. 급여 담당이 "육아휴직 복귀자 급여 예외 처리 어떻게 하죠?" 물어봤는데, 과거 사례 찾아서 수동 계산. 결국 주말에 PPT 만들어서 보고',
        '급여 계산 시 예외 사항 (육아휴직 복귀, 중도 입퇴사, 휴직자 복귀 등) 처리를 수동으로 해서 실수 위험 크고 시간 오래 걸림. 급여 담당 2명이 매월 25일 급여 마감하는데, 전 직원 500명 중 예외 사항이 평균 20-30명. "육아휴직 복귀자는 급여 80%만 지급", "중도 입사자는 일할 계산", "병가 휴직자는 4대보험료만 공제" 등 룰이 복잡함. SAP HR 시스템에서 자동 계산 안 돼서 급여 담당이 Excel에서 수동 계산. 지난 급여 마감 주, 육아휴직 복귀자 A씨 급여를 계산하다가 "복귀 일자"를 잘못 입력해서 급여가 과다 지급됨. A씨가 "급여가 이상하게 많이 나왔어요"라고 전화했는데, 급여 담당이 확인해보니 Excel 수식 오류. 다음 달 급여에서 차감 처리하느라 A씨한테 사과하고 재계산. 이런 실수가 연 3-4회 발생하는데, 직원들 불만 쌓이고 급여 담당은 "실수 안 할까봐" 스트레스',
        '복리후생 만족도를 체계적으로 측정하지 못해서 개선 방향 불명확하고, 예산 배분도 경험에만 의존. 복리후생 항목이 10개 (건강검진, 경조사 지원금, 자녀 학자금, 사내 카페, 동호회 지원, 명절 선물, 휴양소, 장기 근속 포상, 퇴직금, 건강보험) 있는데, "직원들이 뭘 제일 만족하고 뭘 개선 원하는지" 데이터 없음. 조직문화팀이 만족도 조사할 때 "복리후생 만족하세요?"라는 단순 질문만 있어서, "만족한다"는 비율만 나오지 "어떤 항목이 좋고 나쁜지" 세부 데이터 없음. 경영진이 "복리후생 예산 5억인데 효율적으로 쓰고 있어?"라고 물으면, 팀장이 "작년과 동일하게 배분했습니다"라고 답변. "데이터 기반으로 우선순위 정했어?"라는 추가 질문에 답 못함. 결국 "건강검진 예산 늘려달라"는 직원 의견이 게시판에 올라와도 "예산 여유 없어요"라고 답변하는데, 실제로는 다른 항목 예산 줄이면 가능할 수도 있지만 판단 근거 없음',
        '보상/급여/복리후생 담당이 각자 분리되어 데이터 공유 안 돼서 팀 전체 인사이트 도출 어려움. 보상 설계 담당 2명은 시장 조사 데이터만 보고, 급여 담당 2명은 급여 계산만 하고, 복리후생 담당 2명은 복리후생 신청만 처리. 팀장이 "급여와 복리후생을 종합적으로 분석해서 Total Reward 전략 세우자"고 제안했는데, 데이터가 각자 Excel 파일에 분산돼 있어서 통합 어려움. 예를 들어 "급여 수준은 시장 대비 낮은데 복리후생은 좋은가?"라는 질문에 답하려면, 보상 설계 담당한테 시장 조사 Excel 받고, 급여 담당한테 실제 급여 데이터 Excel 받고, 복리후생 담당한테 복리후생 예산 Excel 받아서 팀장이 직접 통합. 이 작업에 주말 이틀 소요. 경영진한테 "우리 회사 Total Reward가 시장 대비 경쟁력 있나요?"라는 질문 받으면 즉답 못하고 "일주일 뒤에 분석 보고드리겠습니다"라고 답변. 5년차 팀장인데 팀 데이터 통합도 못하는 게 자존심 상함'
      ],
      automationNeeds: [
        '시장 조사 데이터 자동 비교 분석',
        '급여 예외 처리 자동화',
