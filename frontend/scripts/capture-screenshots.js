...
async function captureWorkshopScreenshots() {
    console.log('🚀 Starting workshop screenshot capture (pipe mode)...\n');

    let browser;
    try {
        // Puppeteer를 pipe 모드로 실행 (WebSocket 대신)
        browser = await puppeteer.launch({
            headless: 'new',
            pipe: true, // 🔥 WebSocket 대신 pipe 사용
            defaultViewport: {
                width: 1920,
                height: 1080
            },
        ...
        for (let step = 1; step <= 11; step++) {
            ...
            if (step < 11) {
                try {
                    ...
                    const [button] = await page.$x("//button[contains(text(), '빠른 테스트')]");
                    ...
                    if (button) {
                        ...
                        console.log(`   ⚠️  Quick Test button not found at Step ${step}`);
                    }
                } catch (error) {
